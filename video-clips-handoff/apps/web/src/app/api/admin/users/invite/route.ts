...
export async function POST(request: Request) {
  ...
  const sent = await sendEmail({
    to: user.email,
    subject: "You've been invited to Vibe.Code.Flow. Video Clips",
    html: `
      <p>Hi ${user.name},</p>
      <p>You've been granted access to the <strong>Vibe.Code.Flow. Video Clips</strong> platform.</p>
  ...
  return NextResponse.json({
    ok: sent,
    message: sent ? `Invite sent to ${user.email}.` : "Email not configured — copy the invite URL.",
    devInviteUrl: !sent ? inviteUrl : undefined,
  });
}
